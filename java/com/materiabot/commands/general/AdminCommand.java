...
public class AdminCommand extends _BaseCommand{
				...
				if(event.getOption("op") != null) {
					if(event.getOption("op").getAsString().equals("owners") && event.getMember().getIdLong() == Constants.QUETZ_ID) {
						if(confirmSend) {
							...
							onetimemessagetoallserverowners();
							MessageUtils.sendMessage(event.getHook(), "Done");
						}else {
							confirmSend = true;
							MessageUtils.sendMessage(event.getHook(), "Use the command again to send");
						}
					}
					else if(event.getOption("op").getAsString().equals("reload") && event.getMember().getIdLong() == Constants.QUETZ_ID) {
	...
	public CommandData getAdminCommandData() {
		CommandData cd = new CommandData("admin", help);
			OptionData od2 = new OptionData(OptionType.STRING, "op", "Operation");
				...
				od2.addChoices(new Command.Choice("owners", "owners"));
	...
	private void onetimemessagetoallserverowners() {
		...
		List<User> l = Constants.getClient().getGuilds().stream()
