...
public abstract class _AilmentEffect {
	...
	protected final String applyReplaces(Ailment a, int effectIndex, String description, int rank) {
		...
		Integer[] values = type == null ? null : type.getValues(a, effectIndex, rank);
		
		String stack = "";
		if(a.getMaxStacks() > 1 || (type != null && type.getValType() == 16) && values != null) {
		...
		if(values != null)
			for(int i = 0; i < values.length; i++) {
				...
				if(a.getMaxStacks() > 1 || (type != null && type.getValType() == 16))
		...
		while(description.contains("{s") && values != null) { //{s1} = +10 || -10
			...
			String ret = (values[idx] > 0 ? "+" : "") + (a.getMaxStacks() > 1 || (type != null && type.getValType() == 16) ? stack : values[idx]);
		...
		if(type != null && type.getValType() == 16) {
