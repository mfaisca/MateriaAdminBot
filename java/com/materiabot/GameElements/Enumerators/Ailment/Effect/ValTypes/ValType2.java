...
public class ValType2 extends _ValType{
	...
	@Override
	public Integer[] getValues(Ailment ailment, int index) { //Index of effect is on #2 on the args, if no rank_table, then its specific value
		...
		switch(ailment.getValEditTypes()[index]) {
			case 0: //Rank Table direct value
				return new Integer[]{ailment.getRankData().get(ailment.getRankTables()[index]).getValues()[ailment.getArgs()[2]][0]};
			case 1: //Rank Table direct negative value
				return new Integer[]{-ailment.getRankData().get(ailment.getRankTables()[index]).getValues()[ailment.getArgs()[2]][0]};
			case 2: //Rank Table Left Value
				return new Integer[]{ailment.getRankData().get(ailment.getRankTables()[index]).getValues()[ailment.getArgs()[2]][0]};
			case 3: //Rank Table Mid Value
				return new Integer[]{ailment.getRankData().get(ailment.getRankTables()[index]).getValues()[ailment.getArgs()[2]][1]};
			case 4: //Rank Table Right Value
				return new Integer[]{ailment.getRankData().get(ailment.getRankTables()[index]).getValues()[ailment.getArgs()[2]][2]};
